...
<div class="inner-page-wrap">
	...
	<div class="record-nav">
		<ul>
			<li><a href="<?=current_url()?>" class="active"><?=$this->lang->line('trans_wallet')?></a></li>
	...
	<div class="inner-page-xl">
		<ul class="table-type table-flex">
			<li class="table-head">
				<div class="table-keep"><?=$this->lang->line('trans_cash')?></div>
				<div class="table-keep"><?=$this->lang->line('trans_ipoint')?></div>
				<div><?=$this->lang->line('trans_system')?></div>
				<div><?=$this->lang->line('trans_item')?></div>
				<div><?=$this->lang->line('trans_date')?></div>
				<div><?=$this->lang->line('trans_pay_name')?></div>
				<div><?=$this->lang->line('trans_balance_cash')?></div>
                <div><?=$this->lang->line('trans_balance_ipoint')?></div>
				<div class="table-keep"><?=$this->lang->line('trans_desc')?></div>
			</li>
			<?foreach($data as $k => $v){?>
			<li>
				<div class="table-keep"><?=$v['money']?></div>
				<div class="table-keep"><?=$v['ipoint']?></div>
				<div><?=SITE_NAME?></div>
				<div><?=$v['trans_name']?></div>
				<div><?=substr($v['create_date'], 0, 10)?></div>
				<div><?=$v['pay_name']?></div>
				<div><?=$v['wallet_cash']?></div>
                <div><?=$v['wallet_ipoint']?></div>
				<div class="table-keep"><a href="order/record_bag_det/<?=$v['id']?>" class="table-detail"><?=$this->lang->line('trans_desc_link')?></a></div>
			</li>
            <?}?>
		</ul>
        <br /><br />
...
$(document).ready(function(){
    ...
    $('#js-pager').jqPaginator({
        ...
        onPageChange: function (num, type) {
            if(num != <?=$page?>) document.location.href = 'order/record_bag/' + num;
        }
    });
    <?}?>
});

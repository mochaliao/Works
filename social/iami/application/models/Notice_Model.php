...
/*getNoticePost*/
        ...
        public function getNotice($memberID, $limit, $type)//type(leave, collection, reply, share)
        {
                ...
                //         $type="notifes";
                // }
                $this->db->where('notices.member_id', $memberID);
                $this->db->join('members', 'members.member_id=notices.member_id', 'left');
                $this->db->select('notices.id as noticesid, notices.createTime, notices.text, notices.is_read, notices.member_id, members.nickname, members.member_id as memberid');

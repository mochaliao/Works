...
*/
    ...
    public function main($show_type = NULL)
    {
        ...
        $sql = "SELECT p.* FROM posts p LEFT JOIN posts pp ON p.from_post_id=pp.post_id WHERE p.post_id = ? AND ((p.is_deleted = 'Y') OR (pp.is_deleted = 'Y'))";
        array_push($params, $post_id);
        $result = $this->db->query($sql, $params)->num_rows();
//        var_dump($result);
        if($result==1)
        {
        ...
        if(isset($_COOKIE['release_page'])) {
            setcookie('release_page', 'false', strtotime('2018-06-20 23:59:59'));
        }else{
            setcookie('release_page','true');
        }
        ...
        @$data = array(
            ...
            "fansCount" => $fansCount,
            "show_type" => $show_type,
            "release_page" => $_COOKIE['release_page'],
            "isFriend" => true
        );
/*
...
*/
    ...
    public function info($showType = ""){
        ...
        $data = array(
            ...
            'email' => $member["email"],
            'nickname' => $member["nickname"],
            'last_nickname_time' => $member['last_nickname_time'],
        ...
        // 加偉改
        $data['labels'] = $this-> label_model ->getLabel($info_id)->result();
        $data['defaultLabels'] = $this-> label_model ->getLabelALL($member_id)->result();
        ///////////////////
    ...
    public function doEdit()
    {
        ...
        //匿稱規則
        $this->form_validation->set_rules('nickname', $this->lang->line('member_field_nickname'), 'trim|required|max_length[100]|gb_max_length[20]|sensitive['.$this->session->userdata('language_id').']');
        //個人簡歷規則
...
//                redirect('/page/main');
    ...
    public function readNotifies($isEcho=true){
        if ($isEcho && !isLogin(FALSE)){
